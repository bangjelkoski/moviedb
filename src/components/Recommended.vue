		 ...
		 <div class="item-full" v-if="movieLoaded">
		 	...
		 	<div class="item-data">
				...
				<div class="item-info">
					...
					<div class="item-info-col">
						<p>Running Time</p>
						<span class="item-info-value">{{ movie.runtime ? movie.runtime : '-' }}</span>
					</div>
					<div class="item-info-col">
						<p>Box Office</p>
...
<script>
  ...
  export default {
  	...
  	computed: {
  		...
  		formatedRating(){
  			return this.movie.vote_averege ? this.movie.vote_averege + '/ 10' : '-';
  		}
  	},
	created(){
		this.$emit('loadingStart'); // Initiate loading
	...
	beforeDestroy(){
		this.$emit('homeVisited', true); // To load the main background when moving to another route than Home
	}
  }
</script>
